...
using DG.Tweening;                  // Aseet    DOTween (HOTween v2)
using Takap.Utility;
using UnityEngine;
using System;
...
namespace Takap.Samples
{
    ...
    public class RingCmdControl : MonoBehaviour
    {
        ...
        // 移動のインターバル
        [SerializeField] private float magnetSpeed = 0.72f;
        ...
        public int n_remainder;
        public int Abs_n_remainder;
     //   public int n_Rotate = 90;
        ...
        public void TurnRight()
        {
            ...
            this.count++;
            n_remainder = this.count % this.itemList.Count;
            Abs_n_remainder = Math.Abs(n_remainder);
            ...
            seq.Append(DOTween.To(() => this.stepAmount, val => this.stepAmount = val, endValue, this.magnetSpeed));    // DOTween.To(()=> val, x=> val = x, to, duration); valはアニメーションをつけたい値，toはアニメーションの終点の値（valの型に合わせて，intとかVector3とか…），durationはアニメーションの持続時間を表すfloatの値
            seq.SetEase(Ease.OutBounce);
            seq.AppendCallback(() => this.onTurnCompleted());
            this.anim = seq;

            //seq.Append(transform.DORotate(new Vector3(0, 0, n_Rotate), 1f));
            //n_Rotate += 90;
            //    Debug.Log(Abs_n_remainder);
        }
        ...
        private void updateItemsPostion()
        {
            foreach (RingItem item in this.itemList)
            {
                ...
                float deg = (item.InitDegree + this.stepAmount) % 360;
